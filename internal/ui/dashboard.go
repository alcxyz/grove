...
func renderRepoRow(r model.Repo, selected bool, prCount, branchCount int, ciIcon, hlField, hlValue string, nameW, branchW, authorW int) string {
	...
	row := "  " +
		...
		cell(ciIcon, 4) + cell(DimStyle.Render(author), authorW) + DimStyle.Render(ago)
	if selected {
		return selRow(row)
	}
	return row
...
func RenderDashboard(groups []RepoGroup, cursor, width, scrollOffset, maxLines int, prCounts, branchCounts map[string]int, ciStatus map[string]string, hlField, hlValue string) string {
	// fixed: indent(2) + status(8) + sync(10) + pr(4) + br(4) + ci(4) = 32
	...
	header := "  " +
		...
		cell("Sync", 10) + cell("PR", 4) + cell("Br", 4) + cell("CI", 4) + cell("Author", authorW) + "Last Commit"
	...
	for gi, g := range groups {
		...
		for _, r := range g.Repos {
			sw.writeLine(renderRepoRow(r, flatIdx == cursor, prCounts[r.Name], branchCounts[r.Name], ciStatus[r.Name], hlField, hlValue, nameW, branchW, authorW))
			flatIdx++
		}
	}
